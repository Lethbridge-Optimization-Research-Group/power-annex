### defines functions used to create an AC OPF model in JuMP ###
...
defines functions:
init_ac()
solve_model_ac!(ref, model)
************** =#


#=
initializes the JuMP model to the Ipopt optimizer, and returns the model
=#
function init_ac()
...
#= *****************************************************************
Use the reference Dict to add create an AC-OPF JuMP model, run it, and
collect results in a **result** dictionary
ARGS
ref: the reference dictionary containing network data
model: an initialized and empty JuMP model
RET
...
function solve_model_ac!(ref, model)
    ...
    #= ***************************************************************
    add constraints for AC to a JuMP model.
    *************************************************************** =#
    ...
    #= ***************************************************************
    add the objective function for AC-OPF to the model
    *************************************************************** =#
    ...
    # check status and collect the optimal solution
    ...
    for a in ref[:arcs]
        # result[:p_arcs][a] = value(model[:p][a])
        result[:p_arcs][a] = value(p[a])
        #result[:q_arcs][a] = value(model[:q][a])
        result[:q_arcs][a] = value(q[a])
    end
